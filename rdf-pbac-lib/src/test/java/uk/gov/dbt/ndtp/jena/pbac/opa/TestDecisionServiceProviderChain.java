...
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.OpaRequest;
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.OpaResponse;
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.OpaTransport;
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.OpaTransportException;
...
public class TestDecisionServiceProviderChain {
    ...
    private static CxtPBAC newCxt() {
        return CxtPBAC.context(AttributeValueSet.of(java.util.List.of()), Hierarchy.noHierarchy, null);
    }
    ...
    void secondCallWithSameCxt_hitsCacheNotTransport() {
        ...
        CxtPBAC cxt = newCxt(); // ONE CxtPBAC, as PBAC_Request.decideDataset() builds today
        ...
        assertEquals(1, transport.calls.get(),
                "second call sharing the same CxtPBAC must not reach the transport again");
    }
    ...
    void differentCxt_doesNotShareCache() {
        ...
        DecisionContext ctx2 = new DecisionContext(newCxt(), "user-1", "read", "org-a", "dataset1"); // different CxtPBAC = different request

        chain.decide(ctx1, Set.of("public"));
        chain.decide(ctx2, Set.of("public"));

        assertEquals(2, transport.calls.get(),
                "two separate requests (separate CxtPBAC) must each call the transport once");
    }
    ...
    void circuitOpens_evenThroughCachingLayer() {
        ...
        // Each call uses its own CxtPBAC (own request), so caching doesn't mask the
