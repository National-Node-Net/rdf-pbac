...
import uk.gov.dbt.ndtp.jena.pbac.lib.QuadFilter;
...
/*package*/ class SecurityFilterByLabel implements QuadFilter {
    static final Logger logFilter = SysPBAC.DEBUG_LOG;

    private final LabelsGetter labels;
    private final List<String> defaultLookup;
    private final CxtPBAC cxt;
    private final boolean debug;

    // Test and development help (prefer CxtPBAC.systemTrace)
    ...
    SecurityFilterByLabel(DatasetGraph dsgBase, LabelsGetter labels, String defaultLabel, CxtPBAC cxt) {
        this.labels = labels;
        this.defaultLookup = (defaultLabel == null)
                ? List.of(SysPBAC.SYSTEM_DEFAULT_TRIPLE_ATTRIBUTES)
                : List.of(defaultLabel);
        this.cxt = cxt;
        this.debug = generalDebug ? true : cxt.debug();
    }

    // [PBAC] optimize! cache parsing of labels. cache evaluation per-request needed!
    ...
    public boolean test(Quad quad) {
        ...
        if ( dataLabels == null ) {
            ...
            if ( debug )
                FmtLog.info(logFilter, "  No labels configured : %s", SysPBAC.DEFAULT_CHOICE_NO_LABELS);
            return SysPBAC.DEFAULT_CHOICE_NO_LABELS;
        }
    ...
    private static boolean determineOutcome(CxtPBAC cxt, boolean debug, List<String> dataLabels, AttributeValueSet reqAttr) {
    ...
    private static ValueTerm eval1(CxtPBAC cxt, boolean debug, String dataLabel, AttributeValueSet reqAttr) {
