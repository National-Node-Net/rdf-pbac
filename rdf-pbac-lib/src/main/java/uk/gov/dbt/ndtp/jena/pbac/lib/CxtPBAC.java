...
public class CxtPBAC {
    ...
    // same request). Deliberately typed as Object, not a specific type, so CxtPBAC
    ...
    public static CxtPBAC context(AttributeValueSet requestAttributes,
    ...
    private static CxtPBAC context(AttributeValueSet requestAttributes,
                                   HierarchyGetter attrHierarchy,
                                   Map<Attribute, ValueTerm> environment,
                                   DatasetGraph dsgBase) {
        return new CxtPBAC(requestAttributes, attrHierarchy, environment, dsgBase);
    }

    private CxtPBAC(AttributeValueSet requestAttributes, HierarchyGetter attrHierarchy,
                    Map<Attribute, ValueTerm> environment,
                    DatasetGraph baseData) {
        this.requestAttributes = requestAttributes;
        this.attrHierarchy = attrHierarchy;

        this.evalCache = createEvalCache(PBAC.LABEL_EVAL_CACHE_SIZE);
        ...
        // across the data.
        this.hierarchyCache = createHierarchyCache(PBAC.HIERARCHY_CACHE_SIZE);
    ...
    static <X,Y> Cache<X,Y> createEvalCache(int size) {
        ...
        //
        // CxtPBAC is only used within a request execution so the cache is same-thread.
