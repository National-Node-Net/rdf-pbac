...
import uk.gov.dbt.ndtp.jena.pbac.labels.LabelsStoreZero;
...
import uk.gov.dbt.ndtp.jena.pbac.lib.*;

/**
 * Programmatic API to the Policy-Based Access Control (PBAC) functionality.
 *
 * @see SecuredDatasetAssembler
 */
public final class PBAC {

    private PBAC() {
    }

    public final static Logger AzLOG = LoggerFactory.getLogger("uk.gov.dbt.ndtp.jena.pbac.Authz");
    public final static Logger AttrLOG = LoggerFactory.getLogger("uk.gov.dbt.ndtp.jena.pbac.Attribute");
    ...
    /**
     * Test whether a dataset supports PBAC data labelling.
     */
    public static boolean isDatasetPBAC(DatasetGraph dsg) {
        return dsg instanceof DatasetGraphPBAC;
    }

    /**
     * Create a {@link DatasetGraphPBAC}.
     */
    public static DatasetGraphPBAC authzDataset(DatasetGraph dsgBase, LabelsStore labels, String datasetDefaultLabel, AttributesStore attributesStore) {
     ...
     * Create a {@link DatasetGraphPBAC}.
     */
    public static DatasetGraphPBAC authzDataset(DatasetGraph dsgBase, String accessAttributes,
                                                LabelsStore labels, String datasetDefaultLabel,
                                                AttributesStore attributesStore) {
        return new DatasetGraphPBAC(dsgBase, accessAttributes, labels, datasetDefaultLabel, attributesStore);
    }
     ...
     */
    public static DatasetGraph requestDataset(DatasetGraphPBAC dsgAuthz, AttributeValueSet attributes, HierarchyGetter function) {
        CxtPBAC cxt = CxtPBAC.context(attributes, function, dsgAuthz.getData());
     ...
     * This is not used by Fuseki.
     */
    public static DatasetGraph requestDataset(DatasetGraphPBAC dsgAuthz, AttributeValueSet attributes, AttributesStore attrStore) {
        CxtPBAC cxt = CxtPBAC.context(attributes, attrStore, dsgAuthz.getData());
        return filterDataset(dsgAuthz, cxt);
    }
     ...
     * @see #requestDataset
     */
    public static DatasetGraph filterDataset(DatasetGraphPBAC dsgAuthz, CxtPBAC cxt) {
        return PBACRequest.resolveProvider(dsgAuthz).filterDataset(dsgAuthz, cxt);
    }
     ...
     * <p>
     * No {@link DatasetGraphPBAC} is available here, so only the global provider
     * applies (no per-dataset override lookup possible) - matches upstream's
     * equivalent call site.
     */
    public static DatasetGraph filterDataset(DatasetGraph dsgBase, LabelsStore labels, String defaultLabel, CxtPBAC cxt) {
        return PBACRequest.getFilterProvider().filterDataset(dsgBase, labels, defaultLabel, cxt);
    }
    ...
    private static Shapes tryClassPath(String resource, Graph gShacl) {
        try (InputStream in = PBAC.class.getClassLoader().getResourceAsStream(resource)) {
            if (in != null) {
