...
import uk.gov.dbt.ndtp.jena.pbac.lib.VocabAuthzLabels;
        ...
        try {
            while(iter.hasNext() ) {
                ...
                if ( ! G.hasOneSP(graph, subject, VocabAuthzLabels.pPattern) ) {
                    FmtLog.error(SysPBAC.SYSTEM_LOG, "Multiple patterns for same subject:: %s", NodeFmtLib.str(subject, prefixMap(graph)));
                    // XXX throw
                    continue;
                }
                // Pattern
                if ( ! Util.isSimpleString(object) ) {
                    ...
                    FmtLog.error(SysPBAC.SYSTEM_LOG, "Pattern triple does not have a string as the pattern: %s", NodeFmtLib.str(object, prefixMap(graph)));
                    // XXX throw
                    continue;
                }
                ...
                if ( ! isPatternAcceptable(patternStr) ) {
                    FmtLog.error(SysPBAC.SYSTEM_LOG, "Bad pattern: %s: pattern='%s'", NodeFmtLib.str(subject, prefixMap(graph)), patternStr);
                    // XXX throw
                    continue;
                }
...
//                }
                ...
                if ( labels.isEmpty() ) {
                    FmtLog.error(SysPBAC.SYSTEM_LOG, "No labels for pattern: %s", NodeFmtLib.str(subject, prefixMap(graph)));
                    // XXX throw
                    continue;
                }
                ...
                labels.forEach(label-> {
                    if ( ! checkLabel(label) )
                        FmtLog.error(SysPBAC.SYSTEM_LOG, "Bad label: %s : label=%s", NodeFmtLib.str(subject, prefixMap(graph)), label);
                } );
                // OK!
            }
        } finally { iter.close(); }
