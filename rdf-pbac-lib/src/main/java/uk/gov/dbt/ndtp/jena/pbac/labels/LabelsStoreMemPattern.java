...
public class LabelsStoreMemPattern implements LabelsStore {
    ...
    // Future: Consider binding LabelsStore to the DatasetGraphPBAC transactional so
    // that operations on the labels side are also protected.
    // While all operation go through a DatasetGraphPBAC, the dataset is MR+SW (reads can overlap writes).
    ...
    private LabelsStoreMemPattern() {
        // Called from DatasetGraphPBAC in its transaction lifecycle.
    ...
    public List<String> labelsForTriples(Triple triple) {
        ...
        try {
            ...
            return List.of(SysPBAC.DENY_LABEL);
        }
    ...
    public Set<String> distinctLabels() {
        ...
        patternIndex.collectLabels(labels);
        return labels;
    }
