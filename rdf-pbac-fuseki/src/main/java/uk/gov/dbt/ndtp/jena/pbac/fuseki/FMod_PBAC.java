...
import uk.gov.dbt.ndtp.jena.pbac.lib.OpaDatasetFilterProvider;
import uk.gov.dbt.ndtp.jena.pbac.opa.DecisionServiceProvider;
import uk.gov.dbt.ndtp.jena.pbac.opa.OpaDecisionServiceProvider;
import uk.gov.dbt.ndtp.jena.pbac.opa.resilience.CachingDecisionServiceProvider;
...
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.HttpOpaTransport;
import uk.gov.dbt.ndtp.jena.pbac.opa.transport.OpaTransport;

/**
 * Fuseki module for PBAC. This module looks for {@link DatasetGraphPBAC} and ensures
 * certain operations are the correct ones that apply PBAC security.
 ...
 */
public class FMod_PBAC implements FusekiModule {
    public static int LEVEL = 100;

    private final static Logger LOG = PBAC.AzLOG;
    ...
    public FMod_PBAC() {
        this(ServerPBAC.userForRequestFromJwt(), buildDefaultDecisionService());
    }

    private FMod_PBAC(Function<HttpAction, String> getUser, DecisionServiceProvider decisionService) {
     ...
     * default broke 16 of 21 existing tests in TestServerPBAC, which don't have a live
    ...
    private static DecisionServiceProvider buildDefaultDecisionService() {
        ...
        if ( !policyEngineEnabled ) {
            FmtLog.info(Fuseki.configLog, "PBAC: policy engine disabled (POLICY_ENGINE_ENABLED=false)");
            return null;
        }
    ...
    public void prepare(FusekiServer.Builder serverBuilder, Set<String> datasetNames, Model configModel) {
        FmtLog.info(Fuseki.configLog, "PBAC Fuseki Module");

        // Operation registration needs to be in InitFusekiPBAC/SysFusekiPBAC
    ...
    private void prepare1(Builder serverBuilder, String name, Model configModel) {
        ...
        if ( dsg == null )
            return;
        if ( dsg instanceof DatasetGraphPBAC dsgz) {
            FmtLog.info(LOG, "PBAC Dataset: %s", name);
    ...
    public void configDataAccessPoint(DataAccessPoint dap, Model configModel) {
        ...
        if ( ! ( dsg instanceof DatasetGraphPBAC ) ) {
            // Not PBAC.
            return;
        }
        // Replace the standard query handler and upload functions for this DataService
        ActionProcessor procQuery = new PBAC_SPARQL_QueryDataset(getUser);
        ActionProcessor procGSPR = new PBAC_GSP_R(getUser);
        ActionProcessor procUpload = new PBAC_ChangeDispatch();

        // Replace standard processors with PBAC ones.
...
//                    .build();
    ...
    private void warnOperation(DataAccessPoint dap, Operation operation) {
        ...
        dataService.getEndpoints(operation).forEach(ep->{
            FmtLog.warn(Fuseki.configLog, "Update operation for DatasetGraphPBAC mnot recommended");
        });
    }
    ...
    private boolean isAuthzProcessor(ActionProcessor proc) {
        return (proc instanceof PBAC_Processor);
    }
}
