 ...
 */
public class PBAC_DataLoader extends ActionService implements PBAC_Processor {
    ...
    public void validate(HttpAction action) {
        ...
        if ( Lang.TRIG.equals(lang) || Lang.NQUADS.equals(lang) ) {
            ...
            boolean isLabelled = action.getRequestHeader(SysPBAC.H_SECURITY_LABEL) != null;
            if ( ! isLabelled ) {
        ...
        if ( ! ( dsg instanceof DatasetGraphPBAC ) ) {
            FmtLog.warn(action.log, "[%d] Dataset '%s' does not support PBAC security labelling.", action.id, action.getDatasetName());
