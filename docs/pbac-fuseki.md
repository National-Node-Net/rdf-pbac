# RDF PBAC: Fuseki Module
**Repository:** `rdf-pbac`  
**Description:** `Fuseki Module and Security`
<!-- SPDX-License-Identifier: OGL-UK-3.0 -->

The `rdf-pbac-fuseki` module provides triple-level PBAC security for the 
...
* [Fuseki Module for PBAC](#fuseki-module-for-pbac)
  * [Installation](#installation)
  * [Authentication Setup](#authentication-setup)

## Fuseki Configuration

This section describes the PBAC authorization configuration.
...
PBAC security applies to access operations, SPARQL queries and SPARQL Graph Store Protocol read operations (HTTP GET).

Loading data and data labels is performed by a PBAC-specific service.
...
that is using `fuseki:query` instead of `authz:query` if the dataset supports PBAC. This form must
...
A PBAC supporting dataset is of type `authz:DatasetAuthz` and is stacked on top of another dataset
given by the `authz:dataset`.

See [RDF PBAC : User Attribute Store](pbac-user-attribute-store.md) for details of
...
[local user attribute store](pbac-user-attribute-store.md#local-attribute-store)

`authz:attributesURL` refers to a 
[remote user attribute store](pbac-user-attribute-store.md#remote-user-attributes-store)

## Fuseki Module for PBAC
...
Authentication of the request user is required before PBAC authorization can
