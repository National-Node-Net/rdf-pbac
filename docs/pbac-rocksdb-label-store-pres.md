...
-Dpbac.labelstore.biggestfiles=/Users/alan/Downloads/biggest_data_files
```
...
- In the `rdf-pbac-lib` repo, currently the `rocksdb-store-m2` branch
- In `src/main/java` package `uk.gov.dbt.ndtp.jena.pbac.labels`
...
- in `src/test/java` package `uk.gov.dbt.ndtp.jena.pbac`
    - `BulkDirectory` bulk loading tests
- in package `uk.gov.dbt.ndtp.jena.pbac.test` 
    - `NaiveNodeTable`
    - `TrieNodeTable`
    - other helpers
