...
This can be a local store, for when running RDF PBAC as a standalone service or
for a development system,
or a separate service that may be shared, and the PBAC engine needs a URL to
...
The [configuration for the dataset](pbac-fuseki.md#dataset-configuration) needs to include the URL template.
